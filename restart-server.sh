#!/bin/bash
# ============================================
# Script de reinicio automático - Dr. Jorge Hara
# Reinicia el bot cada 10 minutos o si detecta errores
# ============================================

# Usar Node v22 con path absoluto (PM2 no carga NVM profile)
export PATH="/root/.nvm/versions/node/v22.16.0/bin:$PATH"
echo "Now using node $(node -v) (npm v$(npm -v))"

# Aumentar límite de archivos abiertos
ulimit -n 65536

interval=600
restart_count=0
log_file="restart-server.log"
PORT=3008

while true; do
    restart_count=$((restart_count+1))
    timestamp=$(date +"%Y-%m-%d %H:%M:%S")
    echo "[$timestamp] Reinicio #$restart_count" >> "$log_file"

    # Liberar puerto si está ocupado
    echo "Liberando puerto $PORT si está ocupado..."
    PID=$(lsof -t -i:$PORT 2>/dev/null)
    if [ ! -z "$PID" ]; then
        kill -9 $PID 2>/dev/null
        echo "Proceso $PID en puerto $PORT eliminado."
    fi

    # Iniciar el bot en modo desarrollo (nodemon)
    npm run dev &
    server_pid=$!

    # Monitorear por 10 minutos (interval) o hasta que haya crash
    for ((i=0; i<$interval; i++)); do
        sleep 1
        if grep -q "app crashed" restart-server.log 2>/dev/null; then
            echo "App crashed detectado. Reiniciando inmediatamente..." >> "$log_file"
            kill $server_pid 2>/dev/null
            break
        fi
    done

    kill $server_pid 2>/dev/null
    echo "Servidor detenido o reiniciado."
done